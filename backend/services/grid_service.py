...
class GridService:
    ...
    def start_grid(self, config: GridConfig) -> Tuple[Grid, Dict[str, Decimal], int]:
        ...
        if available < required_investment:
            raise InsufficientBalanceError(
                "Insufficient balance for requested investment.",
        ...
        initialization = engine.initialize(calculation)
        order_payloads = []
    ...
    def stop_grid(self, grid_id: str, *, cancel_orders: bool) -> Tuple[Grid, int]:
        ...
        grid.stopped_at = datetime.utcnow()

        cancelled = 0
        if cancel_orders:
            pending_orders = self.order_repository.find_pending_orders(grid_id=grid_id)
    ...
    def get_status(self, *, grid_id: Optional[str] = None) -> GridStatusResponse:
        ...
        levels = self.level_repository.find_by_grid(grid.id)
        orders, _ = self.order_repository.paginate(page=1, limit=1000, grid_id=grid.id)
        total_trades = self.trade_repository.count(filters={"grid_id": grid.id}) if hasattr(self.trade_repository, "count") else 0
        ...
        runtime = None
        if grid.started_at:
            delta = (grid.stopped_at or datetime.utcnow()) - grid.started_at
    ...
    def get_history(self, *, page: int, limit: int) -> Tuple[list[GridHistoryItem], int]:
        grids, total = self.grid_repository.paginate_history(page=page, limit=limit)
        history_items = []
        for grid in grids:
            duration = None
